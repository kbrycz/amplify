  ...
  return (
    <div className={`${className} tabular-nums`}>
      {stringValue.split("").map((digit, index) => (
        <span
          key={index}
          className={`inline-block relative h-[1.1em] overflow-hidden ${digit === "," ? "mr-[0.1em]" : ""}`}
          style={{ width: digit === "," ? "0.1em" : "0.6em" }}
        >
          <span
            className="absolute inset-0 flex flex-col items-center transition-transform duration-500"
            style={{
              transform: digit === "," ? "none" : `translateY(-${Number(digit) * 100}%)`,
            }}
          >
            {digit === "," ? (
          ...
          {digit === "," && <span className="absolute inset-0 flex items-center justify-center">,</span>}
        </span>
      ))}
    </div>
  );
